...
public partial class Details : System.Web.UI.Page {
    ...
    protected void cmdDuplicate_Click(object sender, EventArgs e)
    {
        if (Page.IsValid)
        {
            using (BargeDataContextDataContext context = new BargeDataContextDataContext())
            {
                string oldID = Page.Request.QueryString["ID"].ToString();
                string newID = txtNewID.Text;
                context.csp_Duplicate_Barge(oldID, newID);
